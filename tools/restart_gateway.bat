...
powershell -NoProfile -Command "Remove-Item -Path \"%APPDATA%\openclaw\*.lock\" -Force -ErrorAction SilentlyContinue; Remove-Item -Path \"%LOCALAPPDATA%\openclaw\*.lock\" -Force -ErrorAction SilentlyContinue; Write-Host '锁文件已清除'"
echo     ✅ 锁文件已清除。
...
echo    如果再次出现 "lock timeout"，请手动重启此脚本。
echo.
